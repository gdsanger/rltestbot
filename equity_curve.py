...
def compare_with_buy_and_hold(
    ...
    """Compare agent equity curve with a buy & hold benchmark.
    ...
    testnet : bool, optional
        Whether to use the Binance Spot Testnet, by default ``False``.
    return_plot : bool, optional
        If ``True``, return a ``matplotlib`` figure with both curves.
    close_prices : pandas.Series, optional
        Pre-fetched close prices to use. If ``None`` they will be fetched
        via :func:`fetch_recent_candles`.
    ...
    pandas.DataFrame or matplotlib.figure.Figure
        DataFrame with ``agent`` and ``buy_hold`` columns or a plot if
        ``return_plot`` is ``True``.
    """
    ...
    agent_curve = calculate_equity_curve(
        log_path,
    ...
    amount = initial_capital / close_prices.iloc[0]
    bh_curve = amount * close_prices.iloc[: len(agent_curve)]
    bh_curve = pd.Series(bh_curve, name="buy_hold")
    bh_curve.index = agent_curve.index

    if return_plot:
        import matplotlib.pyplot as plt

        fig, ax = plt.subplots(figsize=(10, 4))
        agent_curve.plot(ax=ax, label="Agent")
        bh_curve.plot(ax=ax, label="Buy & Hold")
        ax.set_xlabel("Step")
        ax.set_ylabel("Equity")
        ax.set_title("Agent vs Buy & Hold")
        ax.legend()
        fig.tight_layout()
        return fig

    return pd.DataFrame({"agent": agent_curve, "buy_hold": bh_curve})
